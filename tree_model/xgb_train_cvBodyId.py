...
def train():
    ...
    pred_prob_y = bst.predict(dtest).reshape(
        test_x.shape[0], 4)  # predicted probabilities
    ...
    df_output.to_csv(
        "../ensemble_learning/result/_{}prob.csv".format(sys.argv[1]), index=False)
...
def cv():
    ...
    pred_prob_holdout_y = clf.predict(dtest).reshape(
        test_y.shape[0], 4)  # probabilities
    ...
    test_df[['Headline', 'Body ID', 'Stance', 'actual', 'predicted', 'prob_0',
...
def show_incorrect_pred(actual, predicted, idx_valid):
    ...
    data['actual'] = actual
    data['articleBody'] = data['articleBody'].map(
        lambda s: s.replace("\n", ""))
    ...
    derr.to_csv('incorrect.csv', columns=[
                'Body ID', 'Stance', 'predicted', 'Headline', 'articleBody'], index=False)
