...
class TfidfFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df):
        ...
        def cat_text(x):
            res = '%s %s' % (' '.join(x['Headline_unigram']), ' '.join(
                x['articleBody_unigram']))
            return res
        ...
        # Tf-idf calculated on the combined training + test set
        vec.fit(df["all_text"])
        vocabulary = vec.vocabulary_

        vecH = TfidfVectorizer(ngram_range=(
            1, 3), max_df=0.8, min_df=2, vocabulary=vocabulary)
        # use ' '.join(Headline_unigram) instead of Headline since the former is already stemmed
        xHeadlineTfidf = vecH.fit_transform(
        ...
        vecB = TfidfVectorizer(ngram_range=(
            1, 3), max_df=0.8, min_df=2, vocabulary=vocabulary)
        xBodyTfidf = vecB.fit_transform(
        ...
        simTfidf = np.asarray(map(cosine_sim, xHeadlineTfidf, xBodyTfidf))[
            :, np.newaxis]
    ...
    def read(self, header='train'):
        ...
        # return [xHeadlineTfidf, xBodyTfidf]
