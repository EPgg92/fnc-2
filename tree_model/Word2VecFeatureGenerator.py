...
class Word2VecFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df):
        ...
        df["Headline_unigram_vec"] = df["Headline"].map(
            lambda x: preprocess_data(x, exclude_stopword=False, stem=False))
        df["articleBody_unigram_vec"] = df["articleBody"].map(
            lambda x: preprocess_data(x, exclude_stopword=False, stem=False))
        ...
        #headlineVec = [0]
        headlineVec = map(lambda x: reduce(np.add, [model[y] for y in x if y in model], [
                          0.] * 300), Headline_unigram_array)
        ...
        #bodyVec = [0]
        bodyVec = map(lambda x: reduce(
            np.add, [model[y] for y in x if y in model], [0.] * 300), Body_unigram_array)
        ...
        simVec = np.asarray(map(cosine_sim, headlineVec, bodyVec))[
            :, np.newaxis]
        print 'simVec.shape:'
        print simVec.shape
