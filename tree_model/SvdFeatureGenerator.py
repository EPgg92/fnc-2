...
class SvdFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df):
        ...
        print featuresTrain, len(featuresTrain)
        ...
        # fit to the combined train-test set (or the full training set for cv process)
        svd.fit(xHBTfidf)
        ...
        simSvd = np.asarray(map(cosine_sim, xHeadlineSvd, xBodySvd))[
            :, np.newaxis]
        print 'simSvd.shape:'
        print simSvd.shape
